...
const ROIPage = () => {
  ...
  const fetchroiData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:5000/api/roi?page=${currentPage}&limit=${itemsPerPage}`);
      if (!response.ok) {
  ...
  const processGraphData = (dailyData) => {
    ...
    const grouped = dailyData.reduce((acc, item) => {
      ...
      if (!acc[dateKey]) {
        acc[dateKey] = { date: dateKey, users: 0, feedback: 0, rawDate: weekStart };
      }
  ...
  const drawChart = () => {
    ...
    const y = d3.scaleLinear()
      .domain([0, d3.max(graphData, d => Math.max(d.users, d.feedback)) * 1.1])
      .nice()
      .rangeRound([height, 0]);
    ...
    svg.append("g")
      ...
      .text(d => `${d.key === 'users' ? 'New Users' : 'Feedback'}: ${d.value}`);
  ...
  const downloadCSV = () => {
    const headers = ["First Name", "Last Name", "Email", "Age", "Gender", "Phone", "Joined Date"];
    const csvContent = [
      headers.join(","),
    ...
    <div className="min-h-screen bg-black text-white p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={downloadCSV}
            className="flex items-center gap-2 bg-[#FAD502] text-black px-4 py-2 rounded-lg font-semibold hover:bg-[#eac702] transition-colors"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className="bg-zinc-900/50 border border-white/10 p-6 rounded-2xl backdrop-blur-sm">
            <div className="flex items-center gap-4 mb-4">
              ...
              <h3 className="text-zinc-400 font-medium">Total Unique Users Visited</h3>
            </div>
          ...
          <div className="bg-zinc-900/50 border border-white/10 p-6 rounded-2xl backdrop-blur-sm">
            ...
            <p className="text-sm text-zinc-500 mt-1">
              {stats.count > 0 ? ((feedbackCount / stats.count) * 100).toFixed(1) : 0}% response rate
            </p>
          </div>
        </div>
        ...
        <div className="bg-zinc-900/50 border border-white/10 p-6 rounded-2xl backdrop-blur-sm mb-10">
          <h2 className="text-xl font-semibold mb-6">User Activity & Feedback Trends</h2>
              ...
              <tbody className="divide-y divide-white/5">
                {stats.users.map((user) => (
                  <tr key={user._id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4 text-zinc-300 truncate max-w-xs" title={user.feedback}>
                      {user.feedback || "-"}
                    </td>
